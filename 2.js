...
class Book {
   ...
   */
  describe = () =>
    `Книга: ${this.title}, автор: ${this.author}, колір обкладинки: ${this.coverColor}`;
}
...
class AudioBook {
  ...
  /**
     ...
     */
  describe = () =>
    `Книга: ${this.title}, автор: ${this.author}, колір обкладинки: ${this.audioLength}`;
}
...
class ProductFactory {
  ...
  static createProduct = (type, options) => {
    if (type === this.TYPE.BOOK) return new Book(options);
    else if (type === this.TYPE.AUDIOBOOK) return new AudioBook(options);
    else {
