...
class AuthProcessor {
  constructor(nextProcessor = null) {
...
class RoleProcessor extends AuthProcessor {
  ...
  // Якщо роль користувача - гість (guest), passkey - guest123 аутентифікація успішна.
...
class CredentialsProcessor extends AuthProcessor {
  ...
  validate = (username, passkey) => {
    if (username === "admin" && passkey === "admin123") {
      ...
      return true;
    } else return this.nextProcessor.validate(username, passkey);
  };
}
...
class ProcessorBuilder {
  ...
  add = (newProcessor) => {
    if (this.firstProcessor === null) {
      this.firstProcessor = newProcessor;
      this.lastProcessor = newProcessor;
    } else {
      this.lastProcessor.setNextProcessor(newProcessor);
      this.lastProcessor = newProcessor;
    }
    return this;
  };
